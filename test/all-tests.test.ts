...
describe.only("Auction Tests", function () {
  ...
  async function createAuctionParams(user1: any) {
    return {
      chargePerUnitTokenInEth: ethers.parseEther("0.25"),
      ...
      auctionStartTime: (await time.latest()) + 600,
  ...
  async function createAndFundAuction(user1: any) {
    ...
    auctionAddress1 = await auctionFactory
      .connect(user1)
    ...
    expect(await token.balanceOf(auctionAddress1)).to.equal(
  ...
  it("should allow the user to update the master model", async function () {
    ...
    await auctionFactory
      .connect(owner)
      .updateMasterModel(await newAuctionModel0.getAddress());
  ...
  it("should allow the user to create a new auction", async function () {
    ...
    expect(await newAuction.auctionStartTime()).to.equal(
      auctionParams.auctionStartTime
    );
    expect(await newAuction.auctionEndTime()).to.equal(
      auctionParams.auctionEndTime
    );
    expect(await newAuction.creator()).to.equal(auctionParams.creator);
    expect(await newAuction.startingBidPrice()).to.equal(
  ...
  it("should fund an auction", async function () {
    ...
    expect(await token.balanceOf(newAuction)).to.equal(
      ethers.parseEther("500")
    );
  });
  ...
  it("should allow a user to enter auctions", async function () {
    ...
    await stablecoin
      .connect(user2)
      .approve(newAuction, ethers.parseEther("100"));

    await time.increaseTo(auctionParams.auctionStartTime);
    await newAuction.connect(user2).buyTokensWithStableCoin("20");

    expect(await newAuction.balances(user2.address)).to.equal(
  ...
  it("should FAIL if a non-creator tries to fund the auction", async function () {
    const { user1, user2 } = await loadFixture(deployContractsFixture);
    const auctionParams = await createAuctionParams(user1);
    auctionAddress1 = await auctionFactory
      .connect(user1)
    ...
    await token
      ...
      .approve(auctionAddress1, ethers.parseEther("500"));
    expect(await token.balanceOf(auctionAddress1)).to.equal(0);
    await expect(newAuction.connect(user2).fundAuction()).to.be.revertedWith(
      "Access Forbidden"
    );
  });
  it("should FAIL is a user attempts to enter the auction before it begins", async function () {
    const { user1, user2 } = await loadFixture(deployContractsFixture);
    const { newAuction, auctionParams } = await createAndFundAuction(user1);
    ...
    // await time.increaseTo(auctionParams.auctionStartTime)
    await expect(
      newAuction.connect(user2).buyTokensWithStableCoin("20")
    ).to.be.revertedWith("Auction is yet to begin!");
  });
  it("should FAIL is user claim tokens before auction ends", async function () {
    const { user1, user2 } = await loadFixture(deployContractsFixture);
    const { newAuction, auctionParams } = await createAndFundAuction(user1);
    ...
    await time.increaseTo(auctionParams.auctionEndTime - 60);
    await expect(
      newAuction.connect(user2).claimPurchasedTokens()
    ).to.be.revertedWith("Claim is possible after auction is expired");
  });
  it("should FAIL if a user tries to enter the auction after it has ended", async function () {
    const { user1, user2 } = await loadFixture(deployContractsFixture);
    const { newAuction, auctionParams } = await createAndFundAuction(user1);
    ...
    await time.increaseTo(auctionParams.auctionEndTime + 60);
    await expect(
      newAuction.connect(user2).buyTokensWithStableCoin("2")
    ).to.be.revertedWith("Auction has ended!");
  });
  it("should FAIL if a non-creator tries to remove unbought tokens after the auction", async function () {
    const { user1, user2 } = await loadFixture(deployContractsFixture);
    const { newAuction, auctionParams } = await createAndFundAuction(user1);
    ...
    await expect(
      newAuction.connect(user2).withdrawUnsoldTokens()
